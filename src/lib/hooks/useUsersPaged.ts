...
import { getUsersPaged, getAdminsPaged, updateUserEnabled } from '@/lib/api/services/userService';
...
export function useAdminsPaged(page: number, size: number) {
  return useQuery({
    queryKey: ['admins', 'paged', page, size],
    queryFn: () => getAdminsPaged(page, size),
    staleTime: 30_000,
  });
}
...
export function useToggleAdminEnabled(page: number, size: number) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, enabled }: { id: number; enabled: boolean }) => updateUserEnabled(id, enabled),
    onMutate: async ({ id, enabled }) => {
      const key = ['admins', 'paged', page, size];
      await qc.cancelQueries({ queryKey: key });
      const prev = qc.getQueryData<any>(key);
      // optimistic update
      if (prev) {
    ...
    onError: (_err, _v, ctx) => { if (ctx?.prev && ctx?.key) qc.setQueryData(ctx.key, ctx.prev); },
    onSettled: () => qc.invalidateQueries({ queryKey: ['admins', 'paged', page, size] }),
  });
}